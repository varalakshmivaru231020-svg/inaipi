              ...
              <div className="flex items-center gap-3">
                {socials.map(({ icon: Icon, href, label }) => (
                  <a key={label} href={href} aria-label={label}
                    className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-[#2563eb] hover:text-white hover:-translate-y-1 transition-all duration-300">
              ...
              <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                ...
                <div>
                  ...
                  <textarea required rows={5} placeholder="Write message"
                    className="w-full px-5 py-3.5 rounded-xl bg-blue-50/50 border border-slate-200 text-[15px] text-[#0f172a] placeholder:text-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all duration-200 resize-none" />
                </div>
